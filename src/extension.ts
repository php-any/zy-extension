...
export function activate(context: vscode.ExtensionContext) {
    ...
    const communicationMode = config.get<string>('languageServer.communicationMode', CommunicationMode.TCP) as CommunicationMode;
    const tcpPort = config.get<number>('languageServer.tcpPort', 8080);
...
// 创建TCP模式的服务器选项
function createTcpServerOptions(serverPath: string, port: number, verbose: boolean, logFile: string): ServerOptions {
    return () => {
...
// 创建STDIO模式的服务器选项
function createStdioServerOptions(serverPath: string, verbose: boolean, logFile: string): ServerOptions {
    const args = ['--stdio'];
    if (verbose) {
        args.push('--verbose');
    }
...
// 启动语言服务器
async function startLanguageServer(mode: CommunicationMode, port?: number): Promise<void> {
    try {
        await client.start();
