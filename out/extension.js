    ...
    return () => {
        return new Promise((resolve, reject) => {
            ...
            socket.on("error", (error) => {
                ...
                reject(new Error(`连接到端口 ${LANGUAGE_SERVER_PORT} 的语言服务器失败：${errorMsg}`));
            });
        });
    };
}
...
function activate(context) {
    ...
    const restartCommand = vscode.commands.registerCommand("origami.restartLanguageServer", async () => {
        try {
            ...
            vscode.window.showErrorMessage(`重新连接折言语言服务器失败：${errorMessage}`);
            // 手动重启失败时，启动自动重连
            if (extensionContext) {
...
// 启动重连机制
function startReconnect(context) {
    if (isReconnecting) {
    ...
    reconnectTimer = setInterval(async () => {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error) || "未知错误";
            console.log(`[Extension] 重连失败: ${errorMessage}，将在 ${RECONNECT_INTERVAL}ms 后重试`);
            // 继续重试，不显示错误消息（避免频繁弹窗）
        }
    }, RECONNECT_INTERVAL);
}
